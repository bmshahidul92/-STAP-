...
function calculateMetrics(original, typed, durationMin, minPassWpm, isBangla = false) {
  ...
  const origChars = isBangla ? origText.split("").filter(c => c.trim() !== "") : Array.from(segmenter.segment(origText), x => x.segment).filter(c => c.trim() !== "");
  const typedChars = isBangla ? typeText.split("").filter(c => c.trim() !== "") : Array.from(segmenter.segment(typeText), x => x.segment).filter(c => c.trim() !== "");
